...
export default function SaathiChatAssistant({
  ...
  const handleSendMessage = (textToSend) => {
    ...
    if (step === 'RECIPIENT') {
      ...
      const parsedAmt = cleanText.replace(/[^0-9.]/g, '');
      setAmount(parsedAmt);
      botReply('confirmTransfer', [parsedAmt, recipient || 'Recipient'], 'CONFIRM', true);
    } else if (step === 'CONFIRM') {
      // User says ok/yes or clicks confirm
      botReply('askOtp', [amount, recipient || 'Recipient'], 'OTP');
    } else if (step === 'OTP') {
      const cleanOtp = cleanText.replace(/\D/g, '');
      setOtp(cleanOtp);
      // Check PIN verification
      if (cleanOtp === actualOtp || cleanText.includes(actualOtp) || cleanOtp.length === 4) {
        onTransferCompleted({ recipient, amount, otp: cleanOtp });
        botReply('successReceipt', [amount, recipient], 'SUCCESS');
      } else {
        // Handle OTP retry or error
        const updatedScore = Math.max(0.1, botConfidence - 0.15);
        setBotConfidence(updatedScore);
        if (!isTrustLayerEnabled && updatedScore < 0.50) {
          onTriggerLockout('🚫 Flagged: Incorrect UPI PIN verification entry under standard anti-bot policy (reCAPTCHA v3 score: 0.31)', 0.31);
        } else if (isTrustLayerEnabled) {
          botReply('trustTokenNotice', [], 'OTP');
        } else {
          setMessages(prev => [...prev, { id: Date.now(), sender: 'saathi', rawText: 'Incorrect PIN. Try again.', timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }]);
        }
      }
    }
  };

  // Proceed to PIN Pad from the in-chat summary card
  ...
  return (
    <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden flex flex-col h-[620px] transition-all">
      {/* Saathi Chat Header */}
      <div className="bg-bgInverse text-textInverse p-4 flex items-center justify-between border-b border-accent">
        <div className="flex items-center gap-3">
          <div className="relative">
            <div className="w-11 h-11 rounded-2xl bg-gradient-to-tr from-accent to-success p-0.5 shadow-md">
              <div className="w-full h-full bg-slate-950 rounded-[14px] flex items-center justify-center text-accent">
                <Bot className="w-6 h-6" />
              </div>
            </div>
            <span className="w-3.5 h-3.5 rounded-full bg-success border-2 border-slate-900 absolute -bottom-0.5 -right-0.5 animate-pulse"></span>
          </div>

          <div>
            <div className="flex items-center gap-2">
              <h2 className="font-bold text-sm sm:text-base tracking-tight text-textInverse flex items-center gap-1.5">
                <span>Saathi</span>
                <span className="text-[11px] font-normal text-accent bg-accentBg px-2 py-0.5 rounded-full border border-accent font-sans">
                  {getDialog('uiAssistantBadge')}
                </span>
              </h2>
            </div>
            <p className="text-[11px] text-accent flex items-center gap-1.5 mt-0.5 font-sans">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="p-2 sm:px-3 sm:py-1.5 rounded-xl bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-textInverse border border-slate-700 text-xs flex items-center gap-1.5 transition-all font-sans font-semibold"
          >
      ...
      <div className="flex-1 p-4 sm:p-6 overflow-y-auto space-y-4 bg-bgPrimary/50">
        {messages.map((msg) => {
            ...
            >
              ...
              <div className={`max-w-[85%] sm:max-w-[75%] space-y-2 ${
                ...
                {msg.isSummaryCard && step === 'CONFIRM' && (
                  <div className="bg-slate-900 text-textInverse border-2 border-accent rounded-3xl p-5 shadow-2xl space-y-4 animate-in zoom-in-95 duration-300">
                    <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                      <div className="flex items-center gap-2">
                        <ShieldCheck className="w-5 h-5 text-accent" />
                        <span className="font-bold text-sm text-accent font-sans">{getDialog('uiAuthCardTitle')}</span>
                      </div>
                      <span className="px-2.5 py-0.5 rounded-full bg-accentBg text-accent border border-accent text-[10px] font-sans font-bold">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div className="bg-slate-950/70 p-2.5 rounded-xl border border-slate-800">
                        <span className="text-[10px] text-slate-500 block font-sans">{getDialog('uiRecipientLabel')}</span>
                        <strong className="text-textInverse text-xs">{recipient || 'Dr. Alan Miller'}</strong>
                      </div>

                      <div className="bg-slate-950/70 p-2.5 rounded-xl border border-slate-800">
                        <span className="text-[10px] text-slate-500 block font-sans">{getDialog('uiTotalAmountLabel')}</span>
                        <strong className="text-accent text-sm font-mono">₹{amount || '350.00'} INR</strong>
                      </div>
                    </div>

                    <div className="bg-accentBg border border-accent rounded-xl p-2.5 text-[11px] text-accent flex items-center gap-2 font-sans">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                      <button
                        ...
                        className="px-3 py-2 rounded-xl text-xs text-textPrimary bg-slate-200 hover:bg-slate-300 font-sans font-semibold"
                      >
                ...
                {msg.isSuccessCard && (
                  <div className="bg-bgInverse text-textInverse border-2 border-success rounded-3xl p-5 shadow-2xl space-y-3 animate-in zoom-in-95 font-sans">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-2xl bg-successBg text-success flex items-center justify-center">
                        <CheckCircle2 className="w-6 h-6 text-success" />
                      </div>
                      <div>
                        <h4 className="font-bold text-sm text-success">{getDialog('uiTransferSuccessTitle')}</h4>
                    ...
                    <button
                      type="button"
                      onClick={handleResetChat}
                      className="w-full py-2.5 rounded-xl bg-success hover:bg-success text-slate-950 font-bold text-xs shadow transition-all flex items-center justify-center gap-1.5"
                    >
                      <RotateCcw className="w-3.5 h-3.5" />
              ...
              {/* User Avatar */}
              {msg.sender === 'user' && (
                <div className="w-8 h-8 rounded-xl bg-slate-800 text-accent flex items-center justify-center shrink-0 shadow-sm mt-1">
            ...
            <>
              {['50', '100', '250', '350', '500'].map((val) => (
                <button
                  ...
                  className="px-3 py-1 rounded-xl bg-white hover:bg-accentBg hover:border-accent text-accent border border-slate-200 text-xs font-mono font-bold transition-all shadow-xs"
                >
                  +₹{val}
                </button>
              ))}
