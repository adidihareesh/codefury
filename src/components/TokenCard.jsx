...
export default function TokenCard({ tokenData }) {
  ...
  return (
    <div className="bg-slate-900/90 border border-teal-500/40 rounded-3xl p-4 sm:p-5 text-white shadow-xl">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-bold bg-teal-400 text-slate-950 flex items-center gap-1">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-xl bg-teal-500/20 text-teal-300 hover:bg-teal-500/30 text-xs font-semibold flex items-center gap-1 transition-colors"
          >
      ...
      {/* Raw String Preview */}
      <div className="mt-3 bg-black/60 rounded-2xl p-2.5 border border-slate-800 font-mono text-[11px] text-slate-400 truncate select-all">
        <span className="text-teal-400 font-bold">{tokenData.rawToken.substring(0, 32)}</span>
        <span className="text-purple-400 font-bold">.{tokenData.rawToken.substring(33, 85)}</span>
        <span className="text-emerald-400 font-bold">.{tokenData.rawToken.substring(86)}</span>
      </div>
      ...
      {isExpanded && (
        <div className="mt-4 pt-3 border-t border-slate-800 grid grid-cols-1 md:grid-cols-2 gap-3 text-xs font-mono">
          <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800">
            <span className="text-teal-400 font-bold text-[10px] uppercase block mb-1">Header (Ed25519)</span>
